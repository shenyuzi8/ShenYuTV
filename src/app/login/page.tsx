...
function LoginPageClient() {
  ...
  return (
    <div className='relative min-h-screen flex items-center justify-center px-4 overflow-hidden'>
      ...
      <div className='relative z-10 w-full max-w-md rounded-3xl bg-white/90 dark:bg-zinc-900/90 backdrop-blur-xl shadow-2xl p-6 sm:p-10'>
        ...
        <form onSubmit={handleSubmit} className='space-y-4'>
          ...
          {(mode === 'register' || mode === 'reset') && (
            <div className='flex flex-col sm:flex-row gap-2'>
              ...
              <button
                ...
                className='w-full sm:w-28 h-12 sm:h-auto bg-green-600 text-white rounded-lg disabled:opacity-50'
              >
          ...
          {error && (
            <p
              className={`text-sm text-center ${error.includes('成功') ? 'text-green-500' : 'text-red-500'}`}
            >
              {error}
            </p>
          )}
          ...
          <button
            type='submit'
            disabled={loading}
            className='w-full h-12 bg-green-600 text-white p-3 rounded-lg font-semibold disabled:opacity-50'
          >
            {loading
              ? '处理中...'
              : mode === 'login'
