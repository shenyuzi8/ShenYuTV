...
// ---- 常量 ----
const PLAY_RECORDS_KEY = 'ShenYuTV_play_records';
const FAVORITES_KEY = 'ShenYuTV_favorites';
const SEARCH_HISTORY_KEY = 'ShenYuTV_search_history';

// 缓存相关常量
const CACHE_PREFIX = 'ShenYuTV_cache_';
...
class HybridCacheManager {
  ...
  private clearAllCache(): void {
    ...
    keys.forEach((key) => {
      if (key.startsWith('ShenYuTV_cache_')) {
...
export async function getSkipConfig(
  ...
  // localStorage 模式
  try {
    const raw = localStorage.getItem('ShenYuTV_skip_configs');
    if (!raw) return null;
...
export async function saveSkipConfig(
  ...
  try {
    ...
    configs[key] = config;
    localStorage.setItem('ShenYuTV_skip_configs', JSON.stringify(configs));
    window.dispatchEvent(
      new CustomEvent('skipConfigsUpdated', {
        detail: configs,
      }),
